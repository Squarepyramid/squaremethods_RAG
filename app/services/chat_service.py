...
- History can also bias how you FRAME an answer, not just what facts you state -- watch for this separately. If recent turns were about one topic (e.g. preventive maintenance) and the CURRENT question is really a different kind of question (e.g. a troubleshooting symptom), answer it as what it actually is. Do not relabel or recast this turn's content to match the earlier topic just because the conversation was recently on it -- e.g. do not present troubleshooting/diagnostic steps as "preventive maintenance" unless the Equipment Knowledge block below actually presents them that way. Categorize content by how it's actually labeled in THIS turn's block, never by which topic the conversation happens to have momentum on.
...
def _build_retrieval_query(current_query: str, history_messages: list) -> str:
    ...
    return "Previous user question(s): " + " | ".join(previous_users) + "\nCurrent question: " + q
